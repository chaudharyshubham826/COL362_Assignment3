...
char *BufferManager::GetPage(PageDescriptor pd)
{
	...
	else
	{
		...
		int slotNo = FindSlot();
		if (slotNo == -1)
			throw NoBufferSpaceException(); //error no free slot could be obtained
...
bool BufferManager::FlushPages(int fd)
{
	...
	{
		if (find(usedList.begin(), usedList.end(), i) == usedList.end())
			continue; // skip free slots
		if (buffers[i].pageDescriptor.fd != fd)
			continue;
...
bool BufferManager::FlushPage(PageDescriptor pd)
{
	...
	for (int i = 0; i < numPages; i++)
	{
		if (find(usedList.begin(), usedList.end(), i) == usedList.end())
			continue; // skip free slots
		if (!(buffers[i].pageDescriptor == pd))
			continue;
		//write back page if dirty
		if (buffers[i].dirty)
		{
